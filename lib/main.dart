...
import 'notifications/received_notification.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(primaryColor: AppColors.colorFromHex(AppColors.primary)),
      home: SplashScreen(),
    );
  }
}
...
listenToNotifications() async {
  try {
    ...
    if (FirebaseAuth.instance.currentUser != null) {
      _listenToNotesNotification();
      _listenToTaskNotification();
    }
  } catch (e) {
    print(e);
  }
...
_listenToNotesNotification() {
  ...
  Query reference = firestore
      .collection('notes')
      .where('receivers', arrayContains: FirebaseAuth.instance.currentUser.uid)
  ...
  reference.snapshots().listen((querySnapshot) async {
    if (querySnapshot.docChanges.isNotEmpty) {
      NotesModel notesModel =
          NotesModel.fromJson(querySnapshot?.docChanges?.first?.doc?.data());

      if (notesModel?.id != await LocalStorage.getLastNotesNotificationId()) {
        _showNotification(
            (getNameByContactNumber(notesModel?.creatorPhone) ??
                    notesModel?.creatorPhone) +
                " Shared you Note",
            notesModel.title);
      }
...
_listenToTaskNotification() {
  ...
  reference1.snapshots().listen((querySnapshot) async {
    if (querySnapshot.docChanges.isNotEmpty) {
      TaskModel notesModel =
          TaskModel.fromJson(querySnapshot?.docChanges?.first?.doc?.data());
      if (notesModel.creatorId != FirebaseAuth.instance.currentUser.uid) {
        if (notesModel?.id != await LocalStorage.getLastTaskNotificationId()) {
          _showNotification(
              (getNameByContactNumber(notesModel?.creatorPhone) ??
                      notesModel?.creatorPhone) +
                  " Assigned You A Task",
